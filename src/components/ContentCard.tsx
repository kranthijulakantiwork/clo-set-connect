...
export default function ContentCard({ item }: ContentCardProps) {
  const getPricingDisplay = () => {
    if (item.pricingOption === PricingOption.Paid && item.price !== undefined) {
      return <span className="text-white font-medium text-lg">${item.price.toFixed(2)}</span>
    }
    if (item.pricingOption === PricingOption.Free) {
      return <span className="text-white font-medium text-sm uppercase">FREE</span>
    }
    if (item.pricingOption === PricingOption.ViewOnly) {
      return <span className="text-white font-medium text-sm uppercase">View Only</span>
    }
    return null
  }
