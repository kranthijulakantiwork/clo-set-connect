...
export const useURLSync = () => {
  ...
  useEffect(() => {
    ...
    const pricingOptions = (params.get("pricing")?.split(",").filter(Boolean) as PricingOption[]) || []
    ...
    if (pricingOptions.length > 0 || searchKeyword || sortBy !== "name" || priceMin !== 0 || priceMax !== 999) {
      dispatch(
        setFiltersFromURL({
          pricingOptions,
          searchKeyword,
