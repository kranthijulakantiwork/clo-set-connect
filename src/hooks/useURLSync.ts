...
export const useURLSync = () => {
  ...
  useEffect(() => {
    ...
    const pricingStrings = params.get("pricing")?.split(",").filter(Boolean) || []
    const pricingOptions = pricingStrings
      .map((str) => Number.parseInt(str, 10))
      .filter((num) => !Number.isNaN(num) && num >= 0 && num <= 2) as PricingOption[]

    const searchKeyword = params.get("search") || ""
    const sortBy = (params.get("sort") as SortOption) || "relevance"
    const priceMin = Number.parseInt(params.get("priceMin") || "0")
    const priceMax = Number.parseInt(params.get("priceMax") || "999")

    if (pricingOptions.length > 0 || searchKeyword || sortBy !== "relevance" || priceMin !== 0 || priceMax !== 999) {
      dispatch(
        setFiltersFromURL({
          pricingOptions,
          searchKeyword,
